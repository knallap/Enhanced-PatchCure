...
parser.add_argument("--model-dir",default='checkpoint',type=str,help="checkpoints directory")
...
model.eval()
def apply_tta_and_predict(model, data, device):
    ...
    for tta_fn in tta_transforms:
        augmented = torch.stack([tta_fn(img.cpu()) for img in data]).to(device)
        preds = model(augmented)
        if preds.ndim > 1:  # logits
            preds = preds.argmax(dim=1)
        all_preds.append(preds)

    # Stack and do majority voting
    stacked_preds = torch.stack(all_preds)  # [T, B]
    final_preds = []
    for i in range(batch_size):
        vals, counts = torch.unique(stacked_preds[:, i], return_counts=True)
...
with torch.no_grad():
	...
	for data,labels in tqdm(val_loader):
		...
		if args.tta:
				output = apply_tta_and_predict(model, data, device)
		else:
				output = model(data)
				if args.undefended:
